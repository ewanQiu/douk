...
douk 不提供任何翻墙能力。已经能访问的话，按下面两种情况配：

| 你的情况 | `proxy` 怎么填 |
|---|---|
| 系统级全局代理（VPN 客户端、WARP、路由器分流） | **留空**。流量已经在系统层走了，douk 不用管 |
| 本地代理客户端（Clash / V2Ray / Shadowsocks 等） | 填它监听的地址，见下 |

```toml
[net]
# 填的是「你本机已在运行的代理客户端」的监听地址。
# 这一项不会凭空创造连通性 —— 没有那个客户端在跑，填什么都是连接被拒。
# 端口去客户端设置里看，Clash 默认 7890，V2Ray 常见 10809。
proxy = "http://127.0.0.1:7890"
```

拿不准就跑 `douk doctor`，它会直接告诉你当前配置下 TikTok 通不通。

线路质量也有影响：住宅 IP 比机房 IP 稳得多，机房段容易吃验证码。
